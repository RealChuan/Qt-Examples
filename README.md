...
### [BatteryQuick](BatteryQuick/) - 电池小部件 (QtQuick)

- 使用 QtQuick 实现的电池指示器

好的，这是对 `Bootstrap` 示例的详细说明部分，您可以将其替换到 README 中。

### [Bootstrap](Bootstrap/) - 开机自启动管理
...
### [BubbleWindow](BubbleWindow/) - 气泡式对话框/工具提示
...
### [CheckBoxStandardItem](CheckBoxStandardItem/) - 可勾选的树形项
...
### [LoadingIndicator](LoadingIndicator/) - 加载动画

- 动画加载指示器
- 支持 GIF 动画
- <img src="LoadingIndicator/picture/LoadingIndicator.jpg" width="400" alt="加载指示器">

### [LogAsynchronous](LogAsynchronous/) - 异步日志系统
...
### [MulClient](MulClient/) - 多线程 TCP 客户端

- 多客户端连接
- 每个客户端一个线程的架构

### [MulServer](MulServer/) - 多线程 TCP 服务器

- 每个客户端一个线程的服务器实现
- 实时 TCP 通信

### [NavigationProgressBar](NavigationProgressBar/) - 导航进度条

- 步骤导航组件
- <img src="NavigationProgressBar/picture/NavigationProgressBar.png" width="90%" alt="导航进度">

### [PasswordLineEdit](PasswordLineEdit/) - 密码输入字段
...
### [ProgressArc](ProgressArc/) - 圆形进度指示器

- 弧线式进度可视化
- <img src="ProgressArc/picture/ProgressArc.png" width="90%" alt="弧形进度">

### [ProgressBar](ProgressBar/) - 圆角进度条

- 自定义样式进度条
- QProgressBar 替代方案
- <img src="ProgressBar/picture/ProgressBar.png" width="90%" alt="圆角进度条">

### [ReactorServer](ReactorServer/) - Reactor 模式服务器

- Reactor 设计模式实现
- 多线程回声服务器

### [ShowInMyComputer](ShowInMyComputer/) - 系统集成

- 在系统位置中显示应用程序
- 防火墙白名单

### [SimpleUdp](SimpleUdp/) - UDP 通信

- UDP 广播和接收
- 简单网络通信示例

### [SlipButton](SlipButton/) - 滑动切换按钮
...
### [SqliteWAL](SqliteWAL/) - SQLite 预写式日志

- 多线程 SQLite 数据库操作
- WAL 模式实现
- 线程安全写入操作

### [TableViewModel](TableViewModel/) - 高级表格视图
...
### [TreeViewModel](TreeViewModel/) - 树形和列表视图
...
### [Validator](Validator/) - 增强型输入验证器

- 改进的 IntValidator 和 DoubleValidator
- 自定义验证规则

### [packaging](packaging/) - 跨平台打包解决方案
...
- **核心文件**:
  ...
  - [package.sh](packaging/macos/package.sh) - 主要打包脚本，这里包含签名过程，[Qt-App macOS 打包](https://github.com/RealChuan/Qt-App/tree/main/packaging/macos)这里没有签名过程
  - [utils.sh](packaging/macos/utils.sh) - 打包辅助函数

##### Windows 打包

- **参考实现**: [Qt-App Windows 打包](https://github.com/RealChuan/Qt-App/tree/main/packaging/windows)
- **核心文件**:
  - [sign.bat](packaging/windows/sign.bat) - 代码签名脚本，支持集成到 Inno Setup
