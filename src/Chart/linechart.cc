...
#include "callout.hpp"
#include "dialogchart.hpp"
#include "normalchartdata.hpp"

class LineChart::LineChartPrivate
{
public:
    explicit LineChartPrivate(ChartView *parent) : q_ptr(parent)
    {
...
LineChart::LineChart(QWidget *parent) : ChartView(parent), d_ptr(new LineChartPrivate(this))
{
    setupUI();
    buildConnect();
}
