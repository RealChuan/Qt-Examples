...
DashBoardWidget::DashBoardWidget(QWidget *parent)
    : QWidget(parent), d_ptr(new DashBoardWidgetPrivate(this))
{
    initAnimations();
...
void DashBoardWidget::drawPointer(QPainter &painter, double minSize)
{
    ...
    const double pw = minSize * (1.0 / 120.0); // 指针半宽
    const double pt = minSize * (1.0 / 75.0);  // 指针尖端偏移
    QPolygonF pts;
    pts << QPointF(-pw, 0) << QPointF(0, -pt) << QPointF(pw, 0) << QPointF(0, radius);
