...
public:
     ...
     *        默认 GET；需以 POST 等动词携带 JSON 动作体下载（如 RPC 风格网关）时，
     *        链式追加 method() 与 body()。
     */
    Q_REQUIRED_RESULT RequestBuilder download(QUrl url, QString filePath);
    /**
     * @brief 上传载荷。默认 PUT；经 method() 可改写动词（如 POST /upload/
     *        一步上传、POST /chunk/ 分块的原始体）。
     */
...
public:
    ...
    /**
     * @brief 改写请求动词。Json/Download 请求有效：Download 默认 GET，
     *        可改为 POST 等以表达"动作在请求体"的 RPC 风格接口；
     *        Upload 仅对原始体载荷（putFile/putData）生效：默认 PUT，可改写为
     *        POST 等（如一步上传 POST /upload/ 的原始体），multipart 恒为 POST。
     */
    RequestBuilder &method(HttpClient::Method method);
    /**
     * @brief 附带 JSON 请求体，配合 method() 表达 POST/PUT + body 的下载。
     *
     * Download 附带 body 时以指定动词发送并自动补 Content-Type，
     * .temp 落盘/Range 续传/进度机制与 GET 下载完全一致；
     * Json 请求等价于入口 json 参数（后者被覆盖）；Upload 忽略此配置。
     */
    ...
    /**
     * @brief 同步发起请求，阻塞直至结束。结果经返回值交付：忽略 onFinished
     *        （已配置时告警，避免双重交付）；onProgress 正常派发——阻塞期间
     *        事件循环在运行，进度信号照常回调，适用于同步下载大文件的进度上报。
     *
